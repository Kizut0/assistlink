...
async function applicants(id, feedback = '') {
  ...
  const rankingAction = (feedback ? `<p class="feedback saved" role="status">✓ ${escapeHtml(feedback)}</p>` : '') + (list.length ? `<div class="rank-action"><button class="button primary" data-rank="${id}">${rankedAt ? 'Re-rank applicants' : 'Rank applicants with AI'}</button><span class="muted">AI scores are a review aid; decisions remain yours.</span></div>` : '');
